...
4. System Architecture & Tech Stack
    ...
    Frontend: React (JavaScript/TypeScript) – Chosen for its ability to handle dynamic, real-time UI updates during fast-paced combat. Frontend state is managed via Zustand utilizing the Facade Design Pattern (useCombatEngine) to isolate strict state machine logic from the UI components. UI styling is managed entirely via Tailwind CSS v4.
...
6. MVP Feature Checklist (Week 6 Deliverable)

    [x] Data Layer Integration: C# Backend successfully fetches, parses, and caches polymorphic monster data from the Open5e API.

    [x] State Machine Architecture: Frontend Zustand store implemented with a strict Facade pattern (useCombatEngine) to isolate complex game logic from UI components.

    [x] Encounter Initialization: DM can search for a monster via the UI and add it to the active combat list.

    [x] Initiative Sorting: System automatically sorts the combatants array descending by initiative roll upon adding a new entity.

    [x] Combat Tracker UI: A visual list rendering the active combatants, displaying their HP, AC, and current status conditions.

    [x] Health Management: DM can apply damage or healing to individual combatants via the UI, preventing HP from dropping below 0 or exceeding max HP.
